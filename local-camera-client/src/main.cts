...
import type { BrowserWindow as BrowserWindowType, IpcMainInvokeEvent, NativeImage, MenuItemConstructorOptions } from 'electron'
import type { SavedAgentConfig } from './local-store.js'
import type { AgentRuntimeState } from './health-server.js'
import type { Channel, LocalAgentStatus } from './types.js'

const { app, BrowserWindow, ipcMain, Menu, Tray, nativeImage } = require('electron') as typeof import('electron')

const preloadPath = join(__dirname, 'preload.cjs')
const agentIconPath = join(__dirname, '../ghost.ico')
...
type TrayVisualState = 'on' | 'degraded' | 'off'
type TrayAction = 'rebind'

interface SessionTelemetrySample {
  timestampIso: string
  runtimeStatus: AgentRuntimeState['status'] | 'offline'
  heartbeatFreshnessSec: number | null
  scannedOperations: number
  hasError: boolean
  localAgentState: LocalAgentStatus['state'] | null
...
let worker: { start(): Promise<void>; stop(): void } | null = null
let runtimeState: AgentRuntimeState | null = null
let healthServer: { close(): void } | null = null
let telemetryTimer: ReturnType<typeof setInterval> | null = null
let tray: InstanceType<typeof Tray> | null = null
let autostartEnabled = false
let sessionTelemetry: SessionTelemetrySample[] = []
let latestDashboardSnapshot: DashboardSnapshot = {
  channel: null,
...
function heartbeatFreshnessSeconds(runtime: AgentRuntimeState | null, channel: Channel | null): number | null {
...
function pushTelemetrySample(channel: Channel | null, error: string | null): void {
  const sample: SessionTelemetrySample = {
...
async function refreshDashboardSnapshot(saved: SavedAgentConfig): Promise<DashboardSnapshot> {
  try {
...
function flushPendingTrayAction(): void {
...
function openDashboard(action?: TrayAction): void {
  if (action) {
...
function getCurrentTrayStatus(): { visual: TrayVisualState; label: string } {
...
function createTrayVariant(visual: TrayVisualState): NativeImage {
...
function getAutostartLaunchConfig() {
  ...
  return {
    executable: process.execPath,
    args: [resolve(app.getAppPath()), '--hidden'],
    workingDirectory: resolve(app.getAppPath()),
  }
}
