...
export function AccountMenu({ fullName, organizationName, role, items, onSelect }: AccountMenuProps) {
  ...
  useEffect(() => {
    ...
    function updateDropdownPosition() {
      ...
      const dropdownWidth = Math.min(Math.max(dropdownRect.width, 240), availableWidth)
      const alignedLeft = triggerRect.right - dropdownWidth
      const left = Math.min(
      ...
      const fitsBelow = preferredTop + dropdownRect.height <= window.innerHeight - viewportPadding
      const top = fitsBelow
        ? preferredTop
